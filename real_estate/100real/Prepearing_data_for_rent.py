...
file_name = '/home/nastja/Dataroot/Price_calculator/flat_for_rent.json'
...
data['floor'] = data['floor'].apply(lambda x: 0 if (x == 'tsokol\'' or x =='bel\'etazh') else x) #tsokol'== 0, bel'etazh == 0
...
data['total_square'] = data['square'].apply(lambda x: translit(x[0],"ru", reversed=True) if (x and not x[0].isdigit()) else 0).astype(np.str).replace(to_replace=' kv.m.', value='', regex=True)
data['living_square'] = data['square'].apply(lambda x: x[1] if (x and len(x)>1 and not x[0].isdigit()) else x[0] if (x and len(x)==2 and x[0].isdigit())  else 0)
data['kitchen_square'] = data['square'].apply(lambda x: x[2] if (x and len(x)==3) else x[1] if (x and len(x)==2 and x[0].isdigit())  else 0)
...
open('100real_for_rent_ready.json','w').writelines(dlist)
